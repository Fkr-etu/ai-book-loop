...
The roadmap mixes **product validation** with **technical implementation**. A checked technical capability does not mean the product exit criterion has been validated with users.

Implemented foundations in the current codebase include:
...
- deterministic chapter linting and hybrid linguistic validation before LLM review;
...
Still requiring product evidence rather than being considered "done": author trust, repeated multi-chapter use, measurable time saved versus a generic LLM, false-positive acceptance thresholds, and willingness to pay.

Known technical follow-ups before stronger production concurrency guarantees:

- close the small crash window between review persistence and workflow checkpoint persistence;

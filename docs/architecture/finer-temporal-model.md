...
Un `STORY_POINT` peut maintenant préciser `end_position` pour représenter un intervalle. Quand `end_position` est absent, le scope reste un point unique.

La comparaison déterministe expose les relations suivantes :

- `BEFORE` : le scope gauche précède le droit ;
- `MEETS` : le premier finit au début du second ;

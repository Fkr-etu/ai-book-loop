...
Canon diagnostics remain distinct from linguistic diagnostics and are not sent through the linguistic contextualizer. `CanonDiagnosticChecker` compares newly supplied text with active Canon facts and reports contradictions without changing Canon.
...
An author-facing projection of a consistency detector finding. It can carry stable identifiers, categories, severity/status and the assertion/evidence needed to explain the finding. It is not a replacement for persisted `Conflict` or `CanonicalFact` state.
...
EVIDENCE / CONFLICTS
    ↓
CONSISTENCY FINDINGS (author-facing)
    ↓
REVIEW DECISION
    ↓
CANONICAL FACTS

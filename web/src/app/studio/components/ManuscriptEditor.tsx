  ...
  return (
    <>
      ...
      <section className="mb-4 flex flex-col gap-3 rounded-lg border border-[#c6c6cd]/40 bg-[#fffdfc] p-4 shadow-xs sm:flex-row sm:items-center sm:justify-between" aria-live="polite">
        ...
        {canDecide && <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
          <button type="button" onClick={handleApprove} className="flex items-center justify-center gap-1.5 rounded bg-[#9a6617] px-3 py-2 text-[11px] font-bold text-white"><Check className="h-3.5 w-3.5" /> Garder ce passage</button>
          <button type="button" onClick={handleReject} className="flex items-center justify-center gap-1.5 rounded border border-[#d98980] bg-white px-3 py-2 text-[11px] font-bold text-[#a33b32]"><X className="h-3.5 w-3.5" /> Ne pas garder</button>
        </div>}
      </section>

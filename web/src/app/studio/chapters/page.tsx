...
export default function ChaptersPage() {
  ...
  const version: ChapterVersion | undefined = useMemo(
    () => versions.find((item) => item.versionNumber === selectedVersion) ||
      versions.find((item) => item.versionNumber === chapter?.currentVersion) ||
      versions[versions.length - 1],
    [versions, selectedVersion, chapter?.currentVersion]
  );
  ...
  const critique = () => run(async () => {
    if (!chapter) return;
    const result = await store.reviewChapter(chapter.number, version?.versionNumber, draftText);
    setReview(result);
  });

  const approve = () => run(async () => {
    if (!chapter) return;
    await store.approveChapter(chapter.number);
  });

  const reject = () => run(async () => {
    if (!chapter) return;
    await store.rejectChapter(chapter.number);
  });
  ...
  return (
    <StudioLayout>
      <div className="flex min-h-[calc(100vh-61px)] flex-col bg-[#f8f5f0] lg:flex-row">
        <main className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6 md:p-10">
          <div className="mx-auto max-w-5xl">
            <header className="mb-6 flex flex-col gap-4 border-b border-[#c6c6cd]/30 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#b87500]">Chapitres</p>
                <h1 className="mt-1 font-playfair text-3xl font-bold text-[#0b1c30]">Rédiger, vérifier, décider</h1>
                <p className="mt-2 max-w-2xl text-sm text-[#5f5e5b]">
                  Chaque version affichée vient de l&apos;état du livre. L&apos;IA propose ; l&apos;auteur décide.
                </p>
              </div>
            ...
            {!chapters.length ? (
              <section className="rounded-lg border border-dashed border-[#c6c6cd] bg-white p-10 text-center">
                <h2 className="font-playfair text-xl font-bold text-[#0b1c30]">Aucun chapitre</h2>
                <p className="mt-2 text-sm text-[#76777d]">Approuvez d&apos;abord le plan, puis créez un chapitre depuis l&apos;espace Plan.</p>
              </section>
            ) : (
              <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
                <aside className="space-y-2">
                  <p className="px-1 font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">Structure persistée</p>
                  {chapters.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setSelectedVersion(item.currentVersion || null);
                        }}
                        className={`w-full rounded-lg border p-3 text-left transition ${active ? "border-[#0b1c30] bg-[#0b1c30] text-white" : "border-[#c6c6cd]/40 bg-white text-[#0b1c30] hover:bg-[#e5eeff]"}`}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-mono text-[10px] font-bold uppercase">Chapitre {item.number}</span>
                          <ChevronRight className="h-3.5 w-3.5" />
                        </div>
                        <div className="mt-1 truncate font-playfair text-sm font-bold">{item.title}</div>
                        <div className={`mt-2 text-[10px] ${active ? "text-[#c6c6cd]" : "text-[#76777d]"}`}>{item.status} · v{item.currentVersion}</div>
                      </button>
                    );
                  })}
                </aside>
                ...
                <section className="min-w-0 rounded-lg border border-[#c6c6cd]/30 bg-white shadow-xs">
                  {chapter && (
                    <>
                      <div className="border-b border-[#c6c6cd]/30 p-4 sm:p-6">
                        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                          <div>
                            <p className="font-mono text-[10px] uppercase tracking-wider text-[#76777d]">Chapitre {chapter.number}</p>
                            <h2 className="mt-1 font-playfair text-2xl font-bold text-[#0b1c30]">{chapter.title}</h2>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button type="button" onClick={critique} disabled={loading || !version} className="inline-flex items-center gap-1.5 rounded border border-[#c6c6cd] bg-[#eff4ff] px-3 py-2 text-xs font-semibold text-[#0b1c30] disabled:opacity-40">
                              <RotateCcw className="h-3.5 w-3.5 text-[#b87500]" /> Critiquer
                            </button>
                          </div>
                        </div>
                      </div>

                      <div className="border-b border-[#c6c6cd]/30 bg-[#f8f9ff] p-3">
                        <div className="flex flex-wrap gap-2">
                          {versions.length ? versions.map((item) => (
                            <button key={item.id} type="button" onClick={() => setSelectedVersion(item.versionNumber)} className={`rounded border px-2.5 py-1.5 text-[11px] font-mono ${item.versionNumber === version?.versionNumber ? "border-[#0b1c30] bg-[#0b1c30] text-white" : "border-[#c6c6cd]/40 bg-white text-[#45464d]"}`}>
                              v{item.versionNumber} · {item.source} · {item.status}
                            </button>
                          )) : <span className="px-1 text-xs text-[#76777d]">Aucune version persistée.</span>}
                        </div>
                      </div>

                      <div className="p-4 sm:p-6">
                        <div className="mb-3 flex items-center justify-between gap-3 font-mono text-[10px] text-[#76777d]">
                          <span>{version ? `Version v${version.versionNumber}` : "Aucune version"}</span>
                          <span>{wordCount} mots</span>
                        </div>
                        <textarea value={draftText} onChange={(event) => setDraftText(event.target.value)} disabled={!version} className="min-h-[480px] w-full resize-y rounded border border-[#c6c6cd]/30 bg-[#fffdfc] p-5 font-merriweather text-sm leading-[1.85] text-[#0f172a] outline-none focus:border-[#0b1c30]" placeholder="Le contenu de la version sélectionnée apparaîtra ici." aria-label="Contenu du chapitre" />

                        {canDecide && (
                          <div className="mt-4 flex flex-wrap gap-2 rounded border border-[#c6c6cd]/30 bg-[#f8f9ff] p-3">
                            <button type="button" onClick={approve} disabled={loading} className="inline-flex items-center gap-1.5 rounded bg-[#9a6617] px-3 py-2 text-xs font-bold text-white disabled:opacity-40"><Check className="h-3.5 w-3.5" /> Approuver</button>
                            <button type="button" onClick={reject} disabled={loading} className="inline-flex items-center gap-1.5 rounded border border-[#d98980] bg-white px-3 py-2 text-xs font-bold text-[#a33b32] disabled:opacity-40"><X className="h-3.5 w-3.5" /> Rejeter</button>
                          </div>
                        )}
                      </div>
                    </>
        ...
        <aside className="w-full shrink-0 border-t border-[#c6c6cd]/30 bg-[#f8f9ff] p-4 lg:w-[320px] lg:border-l lg:border-t-0">
          <div className="sticky top-0 space-y-4">
            <section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4">
              <h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">État</h3>
              <div className="mt-3 space-y-2 text-xs text-[#45464d]">
                <div className="flex justify-between gap-3"><span>Plan approuvé</span><strong>{project.outlineApproved ? "Oui" : "Non"}</strong></div>
            ...
            <section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4">
              <h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">Dernière critique</h3>
              {review ? (
                <div className="mt-3 space-y-3 text-xs text-[#45464d]">
                  <div className="flex items-center justify-between"><span>Score</span><strong>{review.score}/10</strong></div>
                  {review.critique && <p className="rounded bg-[#f8f9ff] p-3 italic">{review.critique}</p>}
                  {review.issues.length > 0 && <div><p className="font-bold">Points à revoir</p><ul className="mt-1 list-disc space-y-1 pl-4">{review.issues.map((issue, index) => <li key={`${issue}-${index}`}>{issue}</li>)}</ul></div>}
                </div>
              ) : <p className="mt-3 text-xs text-[#76777d]">Aucune critique pour la version sélectionnée.</p>}
            </section>

            <section className="rounded-lg border border-dashed border-[#c6c6cd]/50 p-4 text-xs text-[#76777d]">
              Les décisions et versions sont soumises au backend. Cette vue ne transforme pas une proposition en état canonique localement.
            </section>
          </div>
        </aside>
      </div>

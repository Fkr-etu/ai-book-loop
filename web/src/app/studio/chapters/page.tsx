...
export default function ChaptersPage() {
  ...
  const previousVersion = useMemo(() => {
    ...
    return versions.filter((item) => item.versionNumber < version.versionNumber).sort((a, b) => b.versionNumber - a.versionNumber)[0];
  }, [versions, version]);
  const reviewedVersion = chapter?.reviewedVersion;
  const selectedVersionIsReviewed = reviewedVersion === version?.versionNumber;
  ...
  const approve = () => run(async () => { if (!chapter || !version) return; await store.approveChapter(chapter.number, version.versionNumber); });
  ...
  const selectVersion = (versionNumber: number) => {
    ...
    const selected = versions.find((item) => item.versionNumber === versionNumber);
    setReview(selected?.review || null);
  };
  ...
  return (
    <StudioLayout>
      <div className="flex min-h-[calc(100vh-61px)] flex-col bg-[#f8f5f0] lg:flex-row">
        <main className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6 md:p-10">
          <div className="mx-auto max-w-5xl">
            <header className="mb-6 flex flex-col gap-4 border-b border-[#c6c6cd]/30 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#b87500]">Chapitres</p><h1 className="mt-1 font-playfair text-3xl font-bold text-[#0b1c30]">Écrire, relire, décider</h1><p className="mt-2 max-w-2xl text-sm text-[#5f5e5b]">Chaque version est conservée. L&apos;IA propose ; l&apos;auteur relit, compare et décide.</p></div>
              ...
              <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
                ...
                <section className="min-w-0 rounded-lg border border-[#c6c6cd]/30 bg-white shadow-xs">
                  {chapter && <>
                    ...
                    <div className="border-b border-[#c6c6cd]/30 bg-[#f8f9ff] p-3"><div className="flex flex-wrap gap-2">{versions.length ? versions.map((item) => <button key={item.id} type="button" onClick={() => selectVersion(item.versionNumber)} className={`rounded border px-2.5 py-1.5 text-[11px] font-mono ${item.versionNumber === version?.versionNumber ? "border-[#0b1c30] bg-[#0b1c30] text-white" : "border-[#c6c6cd]/40 bg-white text-[#45464d]"}`}>Version {item.versionNumber}{item.versionNumber === chapter.reviewedVersion ? " · relue" : ""}</button>) : <span className="px-1 text-xs text-[#76777d]">Aucune version disponible.</span>}</div></div>
                    <div className="p-4 sm:p-6">
                      {version && <div className="mb-4 rounded border border-[#c6c6cd]/30 bg-[#f8f9ff] p-3 text-xs text-[#45464d]"><div className="flex flex-wrap items-center justify-between gap-2"><span><strong>Version {version.versionNumber}</strong>{selectedVersionIsReviewed ? " · cette version a été relue" : " · cette version n'a pas encore été relue"}</span>{previousVersion && <button type="button" onClick={() => selectVersion(previousVersion.versionNumber)} className="font-semibold text-[#0b1c30] underline underline-offset-2">Voir la version précédente</button>}</div></div>}
                      {previousVersion && version && <details className="mb-4 rounded border border-[#c6c6cd]/30 bg-white"><summary className="cursor-pointer px-3 py-2 text-xs font-semibold text-[#0b1c30]">Comparer avec la version précédente</summary><div className="grid gap-3 border-t border-[#c6c6cd]/30 p-3 md:grid-cols-2"><div><p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-[#76777d]">Version {previousVersion.versionNumber}</p><p className="whitespace-pre-wrap text-xs leading-6 text-[#45464d]">{previousVersion.content}</p></div><div><p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-[#76777d]">Version {version.versionNumber}</p><p className="whitespace-pre-wrap text-xs leading-6 text-[#0f172a]">{version.content}</p></div></div></details>}
                      <div className="mb-3 flex items-center justify-between gap-3 font-mono text-[10px] text-[#76777d]"><span>{version ? `Version ${version.versionNumber}` : "Aucune version"}</span><span>{wordCount} mots</span></div>
                      <textarea value={draftText} onChange={(event) => setDraftText(event.target.value)} disabled={!version || workflowBusy} className="min-h-[480px] w-full resize-y rounded border border-[#c6c6cd]/30 bg-[#fffdfc] p-5 font-merriweather text-sm leading-[1.85] text-[#0f172a] outline-none focus:border-[#0b1c30]" placeholder="Le contenu de la version sélectionnée apparaîtra ici." aria-label="Contenu du chapitre" />
                      {canDecide && !workflowBusy && <div className="mt-4 rounded border border-[#c6c6cd]/30 bg-[#f8f9ff] p-3"><p className="mb-2 text-xs text-[#5f5e5b]">Vous pouvez approuver uniquement la version qui a été relue.</p><div className="flex flex-wrap gap-2"><button type="button" onClick={approve} disabled={loading || !selectedVersionIsReviewed} className="inline-flex items-center gap-1.5 rounded bg-[#9a6617] px-3 py-2 text-xs font-bold text-white disabled:opacity-40"><Check className="h-3.5 w-3.5" /> Approuver la version {version?.versionNumber ?? ""}</button><button type="button" onClick={reject} disabled={loading} className="inline-flex items-center gap-1.5 rounded border border-[#d98980] bg-white px-3 py-2 text-xs font-bold text-[#a33b32] disabled:opacity-40"><X className="h-3.5 w-3.5" /> Rejeter</button></div></div>}
                    </div>
                  </>}
                </section>
              </div>
        ...
        <aside className="w-full shrink-0 border-t border-[#c6c6cd]/30 bg-[#f8f9ff] p-4 lg:w-[320px] lg:border-l lg:border-t-0"><div className="sticky top-0 space-y-4"><section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4"><h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">État du livre</h3><div className="mt-3 space-y-2 text-xs text-[#45464d]"><div className="flex justify-between gap-3"><span>Plan approuvé</span><strong>{project.outlineApproved ? "Oui" : "Non"}</strong></div><div className="flex justify-between gap-3"><span>Versions</span><strong>{versions.length}</strong></div><div className="flex justify-between gap-3"><span>Version actuelle</span><strong>{chapter?.currentVersion || "—"}</strong></div>{reviewedVersion && <div className="flex justify-between gap-3"><span>Version relue</span><strong>v{reviewedVersion}</strong></div>}{activeRun && <div className="flex justify-between gap-3"><span>Génération</span><strong>{activeRun.status} · {activeRun.step}</strong></div>}</div></section><section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4"><h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">Dernière analyse</h3>{review ? <div className="mt-3 space-y-3 text-xs text-[#45464d]"><div className="flex items-center justify-between"><span>Score</span><strong>{review.score}/10</strong></div>{review.critique && <p className="rounded bg-[#f8f9ff] p-3 italic">{review.critique}</p>}{review.issues.length > 0 && <div><p className="font-bold">Points à revoir</p><ul className="mt-1 list-disc space-y-1 pl-4">{review.issues.map((issue, index) => <li key={`${issue}-${index}`}>{issue}</li>)}</ul></div>}</div> : <p className="mt-3 text-xs text-[#76777d]">Aucune analyse pour la version sélectionnée.</p>}</section><section className="rounded border border-[#c6c6cd]/30 bg-white p-4 text-xs text-[#5f5e5b]"><strong>Le cycle de révision</strong><p className="mt-1">Analyse → nouvelle version → comparaison → approbation.</p></section></div></aside>
      </div>
    </StudioLayout>
  );
}

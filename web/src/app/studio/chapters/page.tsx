  ...
  return (
    <StudioLayout>
      <div className="flex min-h-[calc(100vh-61px)] flex-col bg-[#f8f5f0] lg:flex-row">
        <main className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6 md:p-10">
          <div className="mx-auto max-w-5xl">
            <header className="mb-6 flex flex-col gap-4 border-b border-[#c6c6cd]/30 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#b87500]">Chapitres</p><h1 className="mt-1 font-playfair text-3xl font-bold text-[#0b1c30]">Rédiger, vérifier, décider</h1><p className="mt-2 max-w-2xl text-sm text-[#5f5e5b]">Chaque version affichée vient de l&apos;état du livre. L&apos;IA propose ; l&apos;auteur décide.</p></div>
              {chapter && <StudioDecisionState status={chapter.status} />}
            </header>
            {workflowBusy && <div className="mb-4 rounded border border-[#c6c6cd]/40 bg-white p-3 text-sm text-[#0b1c30]" role="status" aria-live="polite">Génération en cours · étape <strong>{activeRun.step}</strong> · tentative {activeRun.attempt || 1}. L&apos;état est suivi par le backend.</div>}
        ...
        <aside className="w-full shrink-0 border-t border-[#c6c6cd]/30 bg-[#f8f9ff] p-4 lg:w-[320px] lg:border-l lg:border-t-0"><div className="sticky top-0 space-y-4"><section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4"><h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">État</h3><div className="mt-3 space-y-2 text-xs text-[#45464d]"><div className="flex justify-between gap-3"><span>Plan approuvé</span><strong>{project.outlineApproved ? "Oui" : "Non"}</strong></div><div className="flex justify-between gap-3"><span>Versions</span><strong>{versions.length}</strong></div><div className="flex justify-between gap-3"><span>Version courante</span><strong>{chapter?.currentVersion || "—"}</strong></div>{activeRun && <div className="flex justify-between gap-3"><span>Workflow</span><strong>{activeRun.status} · {activeRun.step}</strong></div>}</div></section><section className="rounded-lg border border-[#c6c6cd]/30 bg-white p-4"><h3 className="font-mono text-[10px] font-bold uppercase tracking-wider text-[#76777d]">Dernière critique</h3>{review ? <div className="mt-3 space-y-3 text-xs text-[#45464d]"><div className="flex items-center justify-between"><span>Score</span><strong>{review.score}/10</strong></div>{review.critique && <p className="rounded bg-[#f8f9ff] p-3 italic">{review.critique}</p>}{review.issues.length > 0 && <div><p className="font-bold">Points à revoir</p><ul className="mt-1 list-disc space-y-1 pl-4">{review.issues.map((issue, index) => <li key={`${issue}-${index}`}>{issue}</li>)}</ul></div>}</div> : <p className="mt-3 text-xs text-[#76777d]">Aucune critique pour la version sélectionnée.</p>}</section><section className="rounded-lg border border-dashed border-[#c6c6cd]/50 p-4 text-xs text-[#76777d]">Les décisions, versions et l&apos;exécution du workflow sont soumis au backend. Cette vue ne fabrique aucun état métier local.</section></div></aside>
      </div>
    </StudioLayout>
  );
}

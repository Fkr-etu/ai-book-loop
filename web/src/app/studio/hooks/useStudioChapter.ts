...
export function useStudioChapter() {
  ...
  useEffect(() => {
    setSelectedChapterNumber((current) => current !== null && chapters.some((chapter) => chapter.number === current) ? current : chapters[0]?.number ?? null);
  }, [chapters]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    store.getCanonicalContext(activeChapter.number).then((context) => { if (!cancelled) setCanonicalContext(context); }).catch((error: unknown) => {
      if (!cancelled) { console.error("Unable to load canonical context", error); setCanonicalContext(null); setContextError("Impossible de charger les repères de votre histoire."); }
    });
    return () => { cancelled = true; };
  }, [activeChapter?.number, store]);
  useEffect(() => {
    setGrillMessages([]); setGrillResponse(null); setGrillOpen(false); setGrillError(null); setGrillNudge(false); setGrillDismissed(false); grillTurn.current = 0;
  }, [project.id, activeChapter?.id]);
  useEffect(() => {
    if (!activeChapter || grillOpen || grillDismissed || grillNudge || grillResponse || isWorking) return;
    const timer = window.setTimeout(() => setGrillNudge(true), GRILL_IDLE_DELAY_MS);
    return () => window.clearTimeout(timer);
  }, [editorContent, activeChapter?.id, grillOpen, grillDismissed, grillNudge, grillResponse, isWorking]);
  ...
  const askGrill = async (messages = grillMessages) => {
    ...
    try {
      ...
      grillTurn.current = nextTurn; setGrillResponse(response);
  ...
  const answerGrill = async (answer: string) => {
    ...
    setGrillMessages(messages); setGrillResponse(null); await askGrill(messages);
  };
  ...
  return { project, chapters, reviews, activeTab, setActiveTab, selectedChapterNumber, setSelectedChapterNumber, setSelectedVersionNumber, canonicalContext, contextError, editorContent, setEditorContent: handleEditorChange, isWorking, activeChapter, versions, activeVersion, handleGenerateVersion, handleReview, handleApprove, handleReject, canDecide: activeChapter?.status === "needs_review", wordCount: editorContent.trim() ? editorContent.trim().split(/\s+/).length : 0, latestReview: reviews[0], grillMessages, grillResponse, grillOpen, grillLoading, grillError, grillNudge, startGrill, answerGrill, dismissGrill, closeGrill: () => setGrillOpen(false) };
}

  ...
  return (
    <div className="min-h-screen bg-[#f8f5f0] text-[#0f172a] flex flex-col justify-between font-inter selection:bg-[#ffddb8] selection:text-[#0f172a] relative overflow-hidden">
      ...
      <header className="p-6 relative z-10 flex justify-between items-center max-w-5xl mx-auto w-full">
        ...
        <Link href="/login" className="text-xs font-semibold text-[#45464d] hover:text-[#0b1c30]">Déjà inscrit ? Se connecter</Link>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center p-6 relative z-10 my-6">
        <div className="w-full max-w-lg bg-white rounded-xl shadow-sm border border-[#c6c6cd]/30 p-8 flex flex-col">
          <div className="text-center mb-6"><h1 className="font-playfair text-2xl font-bold text-[#0f172a] mb-2">Commencez votre histoire</h1><p className="font-courier text-xs text-[#45464d]">Créez votre compte, puis installez votre univers.</p></div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div><label htmlFor="password" className="block text-xs font-semibold text-[#45464d] mb-1">Mot de passe</label><div className="relative"><Lock className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#76777d]" /><input id="password" name="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setError(null); }} placeholder="Choisissez un mot de passe" required minLength={PASSWORD_MIN_LENGTH} autoComplete="new-password" className="w-full pl-10 pr-10 py-2 text-sm border-b border-[#c6c6cd] focus:border-[#b87500] focus:outline-none bg-transparent" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#76777d]" aria-label={showPassword ? "Masquer" : "Afficher"}>{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div><div className="mt-3 rounded-md bg-[#f8f5f0] p-3"><ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-[11px] text-[#76777d]">{passwordChecks.map((check) => <li key={check.label} className={`flex items-center gap-1.5 ${check.valid ? "text-emerald-700" : ""}`}><Check className={`w-3 h-3 ${check.valid ? "opacity-100" : "opacity-30"}`} /><span>{check.label}</span></li>)}</ul></div></div>
            <button type="submit" disabled={loading || !name.trim() || !email.trim() || !passwordIsValid} className="mt-4 w-full bg-[#0f172a] text-[#f8f5f0] font-semibold text-sm py-3 rounded hover:bg-[#213145] transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><span>{loading ? "Création du compte…" : plan === "free" ? "Créer mon compte" : "Créer mon compte et continuer"}</span><ArrowRight className="w-4 h-4 text-[#ffddb8]" /></button>
          </form>
          <p className="mt-4 text-[11px] text-[#76777d] text-center">En vous inscrivant, vous acceptez nos <Link href="/terms" className="underline">Conditions d'utilisation</Link> et notre <Link href="/privacy" className="underline">Politique de confidentialité</Link>.</p>
        </div>
      </main>
      <footer className="p-6 text-center text-xs text-[#76777d] border-t border-[#c6c6cd]/20 max-w-5xl mx-auto w-full flex flex-col md:flex-row justify-between items-center gap-2 relative z-10"><div>© 2026 Book Loop</div><div className="flex gap-4"><Link href="/login" className="hover:text-[#0f172a]">Connexion</Link><Link href="/pricing" className="hover:text-[#0f172a]">Tarifs</Link><Link href="/terms" className="hover:text-[#0f172a]">Conditions</Link><Link href="/privacy" className="hover:text-[#0f172a]">Confidentialité</Link></div></footer>
    </div>
  );
}

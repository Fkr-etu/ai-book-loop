...
export const metadata: Metadata = {
  ...
  description: "Découvrez comment préserver personnages, chronologie, lieux et règles d’un roman lorsque l’IA génère de nouveaux chapitres.",
  alternates: { canonical: "/guides/garder-coherence-roman-ia" },
};
  ...
  return (
    <main className="min-h-screen bg-[#f8f5f0] text-[#0f172a]"><article className="max-w-3xl mx-auto px-4 sm:px-6 py-16 md:py-24">
      ...
      <div className="mt-12 space-y-10 text-[#30323a] leading-relaxed">
        <section><h2 className="font-playfair text-2xl font-bold text-[#0b1c30]">Centralisez les faits importants</h2><p className="mt-3">Gardez une référence des informations qui ne doivent pas être réinventées à chaque chapitre : identité, apparence, relations, lieux, chronologie, règles et événements majeurs. <Link className="font-semibold text-[#0b1c30]" href="/guides/bible-narrative">Découvrez le rôle d’une bible narrative.</Link></p></section>
        ...
        <section><h2 className="font-playfair text-2xl font-bold text-[#0b1c30]">Contrôlez les contradictions</h2><p className="mt-3">Avant de valider un chapitre, recherchez les écarts avec la référence : couleur des yeux, emplacement d’un lieu, ordre des événements, motivations ou relations. Les petites incohérences deviennent coûteuses lorsqu’elles se propagent. <Link className="font-semibold text-[#0b1c30]" href="/guides/eviter-contradictions-roman">Voir la méthode anti-contradictions.</Link></p></section>
        ...
        <section className="bg-white rounded-2xl border border-[#c6c6cd]/40 p-6"><h2 className="font-playfair text-2xl font-bold text-[#0b1c30]">Une bonne boucle de continuité</h2><p className="mt-3">Écrivez le chapitre, vérifiez-le contre votre référence, décidez des changements, puis continuez avec cette version validée. C’est le principe du Canon dans Book Loop.</p><p className="mt-3"><Link className="font-bold text-[#0b1c30]" href="/guides/ecrire-un-roman-avec-ia">Lire la méthode complète pour écrire un roman avec l’IA →</Link></p></section>
      </div>
    </article></main>
  );
}

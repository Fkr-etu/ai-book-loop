...
function toCanonicalContext(context: Awaited<ReturnType<typeof realApiClient.getChapterContext>>): CanonicalContextResponse {
  const globalOutline = context.globalOutline?.chapters.map((chapter) => `## Chapitre ${chapter.number}: ${chapter.title}\nObjectif: ${chapter.objective}\n${chapter.synopsis}`).join("\n\n") ?? "";
...
export class RealBookApi implements BookApi {
  async getBook(id = EMPTY_BOOK_ID): Promise<BookState> { if (id === EMPTY_BOOK_ID) return emptyBook(); return adaptBackendBook(await realApiClient.getBook(id)); }
  ...
  async generateChapter(id: string, chapterNumber: number): Promise<{ book: BookState; versionNumber: number; content: string; run: BackendWorkflowRun }> {
    const run = await realApiClient.generateChapter(id, chapterNumber);
    return { book: adaptBackendBook(await realApiClient.getBook(id)), versionNumber: run.run.attempt, content: run.run.draft, run: run.run };
  }
  async getChapterWorkflowRun(id: string, chapterNumber: number, runId: string): Promise<BackendWorkflowRun> { return realApiClient.getChapterWorkflowRun(id, chapterNumber, runId); }
  async reviewChapter(id: string, chapterNumber: number, versionNumber?: number, draftText?: string): Promise<{ book: BookState; review: SceneReview }> { const result = await realApiClient.reviewChapter(id, chapterNumber, versionNumber, draftText); return { book: adaptBackendBook(result.book), review: result.review as SceneReview }; }
  async approveChapter(id: string, chapterNumber: number): Promise<BookState> { return adaptBackendBook(await realApiClient.approveChapter(id, chapterNumber)); }
  async rejectChapter(id: string, chapterNumber: number): Promise<BookState> { return adaptBackendBook(await realApiClient.rejectChapter(id, chapterNumber)); }
  async getCanonicalContext(id: string, chapterNumber: number): Promise<CanonicalContextResponse> { return toCanonicalContext(await realApiClient.getChapterContext(id, chapterNumber)); }

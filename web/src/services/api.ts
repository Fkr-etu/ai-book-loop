...
export class MockBookApi implements BookApi {
  ...
  async updateBook(id: string, updates: Partial<BookState>): Promise<BookState> { const updated = { ...loadStorageProject(), ...updates }; saveStorageProject(updated); return updated; }
  ...
  async addChapter(id: string, title: string, objective: string): Promise<BookState> { const book = loadStorageProject(); if (!book.outlineApproved) throw new Error("L'outline doit être approuvé avant d'ajouter un chapitre."); const nextNum = (book.chapters || []).length + 1; const chapter: Chapter = { id: `ch-00${nextNum}`, number: nextNum, title, objective, status: "draft", currentVersion: 0, versions: [], scenes: [] }; book.chapters = [...(book.chapters || []), chapter]; saveStorageProject(book); return book; }
  async generateChapter(id: string, chapterNumber: number): Promise<{ book: BookState; versionNumber: number; content: string }> { const book = loadStorageProject(); const ch = (book.chapters || []).find(c => c.number === chapterNumber); if (!ch) throw new Error(`Chapitre ${chapterNumber} introuvable.`); const versionNumber = (ch.currentVersion || 0) + 1; const content = `[Version ${versionNumber}] La lueur d'obsidienne vacilla dans le silence absolu. Le chapitre ${chapterNumber} s'ouvrait sur l'inconnu.`; ch.currentVersion = versionNumber; ch.status = "approved"; ch.summary = `Résumé canonique du chapitre ${chapterNumber}.`; ch.versions = [...(ch.versions || []), { id: `v-00${versionNumber}`, versionNumber, content, createdAt: new Date().toISOString(), source: "ai", status: "approved" }]; saveStorageProject(book); return { book, versionNumber, content }; }
  ...
  async createCharacter(id: string, char: Omit<Character, "id">): Promise<BookState> { const book = loadStorageProject(); const newChar = { ...char, id: `char-${Date.now()}` }; book.characters = [...(book.characters || []), newChar]; saveStorageProject(book); return book; }
  ...
  async createLoreItem(id: string, item: Omit<LoreItem, "id">): Promise<BookState> { const book = loadStorageProject(); const newItem = { ...item, id: `lore-${Date.now()}` }; book.loreItems = [...(book.loreItems || []), newItem]; saveStorageProject(book); return book; }

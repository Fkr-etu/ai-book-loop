...
export function LegalPage({
  ...
  const draft = legal.status !== "ready";
    ...
    <div className="min-h-screen bg-[#f8f5f0] text-[#0f172a] font-inter">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 md:py-16">
        ...
        <article className="bg-white border border-[#c6c6cd]/40 rounded-2xl p-6 sm:p-10 space-y-8 leading-relaxed text-sm">
          {children}
          <aside className="rounded-lg bg-[#fff8ed] border border-[#b87500]/20 p-4 text-xs text-[#5f5e5b]" role="status">
            <strong className="text-[#2a1700]">
              {draft ? "Document en préparation — non finalisé pour commercialisation" : "Document validé pour publication"}
            </strong>
            <br />
            Version du contenu : {LEGAL_CONTENT_VERSION}.
            {draft && " Les données d'identification, conditions contractuelles et textes définitifs doivent être complétés et validés avant commercialisation."}
          </aside>
        </article>
      </main>
      <LegalFooter />

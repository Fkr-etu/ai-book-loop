  ...
  return (
    <header className="sticky top-0 z-50 bg-[#f8f9ff]/90 backdrop-blur-md border-b border-[#c6c6cd]/30 px-4 md:px-6 py-3 flex items-center justify-between">
      ...
      <nav className="hidden md:flex items-center gap-1 bg-[#eff4ff] p-1 rounded-md border border-[#c6c6cd]/20">
        ...
        <Link href="/pricing" className={`px-3 py-1.5 rounded text-xs font-semibold flex items-center gap-2 transition-all ${pathname === "/pricing" ? "bg-[#0b1c30] text-[#ffffff]" : "text-[#45464d] hover:text-[#0b1c30] hover:bg-[#e5eeff]"}`}>
      ...
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-[#f8f9ff] border-b border-[#c6c6cd]/40 p-4 shadow-lg flex flex-col gap-3 md:hidden z-50">
          ...
          <nav className="flex flex-col gap-1">
            <Link href="/dashboard" onClick={() => setMobileMenuOpen(false)} className={`px-3 py-2 rounded text-xs font-semibold flex items-center gap-2 transition-all ${pathname === "/dashboard" ? "bg-[#0b1c30] text-[#ffffff]" : "text-[#45464d] hover:bg-[#e5eeff]"}`}><LayoutDashboard className="w-4 h-4" /> Mes Livres</Link>
            <Link href="/studio" onClick={() => setMobileMenuOpen(false)} className={`px-3 py-2 rounded text-xs font-semibold flex items-center gap-2 transition-all ${pathname.startsWith("/studio") && pathname !== "/studio/export" ? "bg-[#0b1c30] text-[#ffffff]" : "text-[#45464d] hover:bg-[#e5eeff]"}`}><BookOpen className="w-4 h-4" /> Atelier</Link>
            <Link href="/studio/export" onClick={() => setMobileMenuOpen(false)} className={`px-3 py-2 rounded text-xs font-semibold flex items-center gap-2 transition-all ${pathname === "/studio/export" ? "bg-[#0b1c30] text-[#ffffff]" : "text-[#45464d] hover:bg-[#e5eeff]"}`}><Download className="w-4 h-4 text-[#b87500]" /> Exportation</Link>
            <Link href="/pricing" onClick={() => setMobileMenuOpen(false)} className={`px-3 py-2 rounded text-xs font-semibold flex items-center gap-2 transition-all ${pathname === "/pricing" ? "bg-[#0b1c30] text-[#ffffff]" : "text-[#45464d] hover:bg-[#e5eeff]"}`}><CreditCard className="w-4 h-4" /> Tarification</Link>
          </nav>
        </div>
      )}
    </header>

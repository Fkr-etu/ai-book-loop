...
function labelForStep(step: string | null): string {
  ...
  if (step === "analyzing") return "Vérification de la continuité en cours";
  return "Vérification en cours";
}

function severityLabel(severity: string): string {
  if (severity === "high" || severity === "critical") return "À vérifier en priorité";
  if (severity === "medium") return "À vérifier";
  return "Point à examiner";
}
...
export function ConsistencyAnalysisPanel({ bookId }: { bookId: string }) {
  ...
  useEffect(() => {
    ...
    if (!jobId) return;
    setLoading(true);
    void refreshJob(jobId).catch(() => setError("Impossible de retrouver l’état de la vérification.")).finally(() => setLoading(false));
  }, [bookId, refreshJob]);

  useEffect(() => {
    if (!job || TERMINAL.has(job.status)) return;
    const timer = window.setInterval(() => {
      void refreshJob(job.job_id).catch(() => setError("Impossible d’actualiser la vérification."));
    }, 2000);
  ...
  const start = async () => {
    ...
    try {
      ...
      setError(err instanceof RealApiError ? err.message : "La vérification n’a pas pu être lancée.");
    } finally {
      setStarting(false);
    }
  };
  ...
  const submitFeedback = (value: "useful" | "not_useful") => {
    ...
    setFeedback(value);
    track("analysis_finding_feedback", { feedback: value, issue_count: (job.result?.issues ?? []).length });
  };
    ...
    <section className="bg-white rounded-xl border border-[#c6c6cd]/40 overflow-hidden" aria-labelledby="consistency-analysis-title">
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#c6c6cd]/20">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 id="consistency-analysis-title" className="font-playfair font-bold text-lg text-[#0b1c30]">Vérifier la continuité</h2>
      ...
      {job && <div className="p-4 sm:p-5 space-y-4">
        {running && <div aria-live="polite" className="space-y-2"><div className="flex justify-between text-xs text-[#45464d]"><span>{job.status === "queued" ? "En attente" : labelForStep(job.current_step)}</span><span>{job.progress}%</span></div><div className="h-2 bg-[#ececf0] rounded-full overflow-hidden"><div className="h-full bg-[#b87500] transition-all" style={{ width: `${Math.min(100, Math.max(0, job.progress))}%` }} /></div><p className="text-[11px] text-[#76777d]">Vous pouvez quitter cette page : la vérification continue en arrière-plan.</p></div>}
        {job.status === "succeeded" && <div className="flex items-center gap-2 text-xs font-semibold text-[#0b1c30]"><CheckCircle2 className="w-4 h-4 text-[#3f7d55]" />Vérification terminée — {issues.length} point{issues.length > 1 ? "s" : ""} à examiner.</div>}
        {job.status === "failed" && <div role="alert" className="text-xs text-[#5c2020]">La vérification n’a pas abouti. {job.error_message || "Vous pouvez la relancer."}</div>}
        {job.status === "cancelled" && <div className="text-xs text-[#5f5e5b]">La vérification a été annulée.</div>}
        {issues.length > 0 && <div className="space-y-2">{issues.map((issue) => <article key={issue.id} className="rounded-lg border border-[#c6c6cd]/30 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-semibold text-[#0b1c30]">{issue.message}</p><span className="text-[10px] font-semibold text-[#76777d] shrink-0">{severityLabel(issue.severity)}</span></div><p className="text-[11px] text-[#5f5e5b] mt-2">{issue.left_statement}</p><p className="text-[11px] text-[#5f5e5b] mt-1">{issue.right_statement}</p></article>)}</div>}

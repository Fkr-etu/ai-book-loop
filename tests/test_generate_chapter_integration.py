...
class RecordingLLM:
    def __init__(self, drafts: list[str] | None = None, review_scores: list[int] | None = None) -> None:
...
def make_outline(*chapters: tuple[int, str, str]) -> Outline:
    return Outline(chapters=[{"number": number, "title": title, "objective": objective} for number, title, objective in chapters])
...
def test_generate_chapter_passes_canonical_context_through_real_workflow() -> None:
    book = BookState(
        ...
        chapters=[
            Chapter(id="c1", number=1, title="Return", objective="Introduce the protagonist's return.", status=ChapterStatus.APPROVED, summary="The protagonist returns to the capital and hides their identity.", current_version=1),
            Chapter(id="c2", number=2, title="The secret", objective="Reveal the first clue about the forbidden magic."),
        ],
    )
...
def test_generate_chapter_retries_after_linter_failure_and_preserves_history() -> None:
    ...
    assert [version[3] for version in repository.versions] == ["TODO: unfinished draft", "A corrected chapter draft."]

...
class BookUsage:
    ...
    def register_book_identity(self, *, book_id: str, identity: str) -> None:
        self.identities.setdefault(book_id, identity)
...
def test_free_capacity_is_shared_by_book_identity_across_accounts() -> None:
    ...
    for index in range(5):
        use_case.start(book_a if index % 2 == 0 else book_b, 1, idempotency_key=f"attempt-{index}")

    with pytest.raises(PermissionError, match="Free-tier capacity for this book"):
        use_case.start(book_b, 1, idempotency_key="attempt-5")
...
def test_free_book_identity_survives_book_edits() -> None:
    ...
    assert usage.get_book_identity(book_id=book_a.id) == usage.get_book_identity(book_id=book_b.id)

...
def test_coverage_exposes_each_detector_filter_without_changing_rules() -> None:
    ...
    assert coverage.same_subject_pairs == 31
    assert coverage.same_predicate_pairs == 21
    assert coverage.different_object_pairs == 19
    assert coverage.action_event_excluded == 1
    assert coverage.multi_valued_excluded == 1
    assert coverage.temporal_missing_scope == 10
    assert coverage.temporal_non_overlapping == 1
    assert coverage.temporal_overlapping == 6
    assert coverage.final_candidates == 6


def test_coverage_ignores_rejected_assertions_when_called_with_filtered_input() -> None:

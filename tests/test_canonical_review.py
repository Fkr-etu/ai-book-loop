...
def _seed_assertions(repository: PostgresBookRepository) -> tuple[Assertion, Assertion]:
...
def test_detects_competing_subject_predicate_values():
...
def test_detection_is_idempotent_and_returns_persisted_conflict():
    repository = _repository()
    left, right = _seed_assertions(repository)
...
def test_resolved_conflict_is_not_reopened_by_detection():
    repository = _repository()
...
def test_acceptance_creates_versioned_canonical_fact_and_audit_decision():
    repository = _repository()
    left, right = _seed_assertions(repository)
    DetectConflicts(repository).execute(book_id="book-1")

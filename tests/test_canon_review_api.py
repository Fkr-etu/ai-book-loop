...
def setup_client(tmp_path):
    settings = Settings(database_url=f"sqlite:///{tmp_path / 'canon.db'}", auth_secret_key=TEST_SECRET)
    container = Container(settings=settings)
    client = TestClient(create_app(container))
    response = client.post(
        "/api/auth/register",
        json={"email": "canon@example.com", "password": "password123", "name": "Canon Reviewer"},
    )
    assert response.status_code == 201
    book = client.post(
        "/api/books",
        json={"title": "Canon API", "theme": "Mystery", "author_idea": "Test", "lore": "Lore"},
    )
...
def test_canon_review_endpoints_expose_conflicts_and_facts(tmp_path):
    client, container, book_id = setup_client(tmp_path)
    ...
    container.repository.save_conflict(
        Conflict(
            id="conflict-1",
            book_id=book_id,
            left_assertion_id=first.id,
            right_assertion_id=second.id,
...
def test_accept_review_promotes_assertion_to_canonical_fact(tmp_path):
    client, container, book_id = setup_client(tmp_path)
    assertion = seed_assertion(container, book_id)

    response = client.post(
        f"/api/books/{book_id}/assertions/{assertion.id}/review",
        json={"decision": "accept", "rationale": "Validated by author"},
    )
...
def test_reject_and_defer_do_not_create_canonical_facts(tmp_path):
    client, container, book_id = setup_client(tmp_path)
    ...
    assert client.post(
        f"/api/books/{book_id}/assertions/{rejected.id}/review",
        json={"decision": "reject", "rationale": "Contradicted by source"},
    ).status_code == 200
    assert client.post(
        f"/api/books/{book_id}/assertions/{deferred.id}/review",
        json={"decision": "defer", "rationale": "Need more evidence"},
    ).status_code == 200
...
def test_canon_review_api_respects_book_ownership(tmp_path):
    client_a, container, book_id = setup_client(tmp_path)
    assertion = seed_assertion(container, book_id)

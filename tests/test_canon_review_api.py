...
from book_loop.domain.models import Assertion, AssertionStatus, CanonicalFact, Conflict, ConflictStatus, DocumentChunk, Evidence, SourceDocument
...
def test_analyze_canon_change_api_returns_evidence_backed_findings():
    ...
    chunk = DocumentChunk(id="chunk-impact", source_document_id=source.id, content=source.content, sequence=0, start_offset=0, end_offset=len(source.content))

from book_loop.application.use_cases.grill import (
...
class RecordingLLM:
    ...
    def generate_structured(self, **kwargs):
        ...
        return GrillResponse(
            reply="Je vois une faiblesse dans cette motivation.",
            question="Qu'est-ce qui empêche réellement ton personnage de partir ?",
            done=False,
        )
...
def make_book(personality: GrillPersonality = GrillPersonality.CHALLENGER) -> BookState:
    return BookState(
        ...
        theme="Roman · Policier",
        author_idea="Une inspectrice reprend une vieille affaire.",
        creative_brief=CreativeBrief(premise="Une inspectrice reprend une vieille affaire."),
        lore="Paris en hiver.",
        constraints=["Rester réaliste"],
        grill_personality=personality,
    )


def test_grill_returns_minimal_structured_contract_and_uses_personality() -> None:
    llm = RecordingLLM()
    result = Grill(llm).execute(
        book=make_book(GrillPersonality.DEVILS_ADVOCATE),
        messages=[GrillMessage(role="user", content="Elle poursuit l'enquête par devoir.")],
        turn=1,
    )

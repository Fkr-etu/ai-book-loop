...
def _seed(repository: PostgresBookRepository, *, value: str = "32", assertion_id: str = "assertion-1") -> Assertion:
...
def test_acceptance_is_idempotent_and_history_is_immutable() -> None:
...
def test_replacing_canonical_fact_creates_a_new_active_version() -> None:
    repository = PostgresBookRepository(DATABASE_URL)  # type: ignore[arg-type]
    try:
        first = _seed(repository, value="32", assertion_id="assertion-1")
...
def test_database_rejects_two_active_facts_for_same_subject_and_predicate() -> None:
    repository = PostgresBookRepository(DATABASE_URL)  # type: ignore[arg-type]
    try:
        first = _seed(repository, value="32", assertion_id="assertion-1")
        ReviewAssertion(repository).execute(book_id="book-1", assertion_id=first.id, decision=ReviewDecisionType.ACCEPT)
        ...
        with pytest.raises(Exception):
            repository._connection._connection.execute(
                ...
                INSERT INTO canonical_facts(
                    id, book_id, assertion_id, statement, subject, predicate, object,
                    decision_id, version, active, previous_fact_id
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """,
                (
                    "illegal-fact", "book-1", "assertion-2", "Alice is 33.", "Alice", "age", "33",
                    "illegal-decision", fact.version + 1, True, fact.id,
                ),
            )
    finally:

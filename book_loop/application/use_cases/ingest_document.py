...
class IngestDocument:
    ...
    def execute(
        ...
        *,
        ...
        existing = self._repository.find_source_by_hash(
            book_id=book_id, content_hash=content_hash
        )
        ...
        for chunk in chunks:
            ...
            for raw in self._extractor.extract(chunk=chunk):
                extracted = raw if isinstance(raw, ExtractedAssertion) else ExtractedAssertion.model_validate(raw)
                ...
                item = Assertion(
                    ...
                    predicate=extracted.predicate.strip(),
                    object=extracted.object.strip(),
                ...
                ev = Evidence(
                    ...
                    source_document_id=source.id,
                    chunk_id=chunk.id,
                    start_offset=chunk.start_offset + extracted.start_offset,
                    end_offset=chunk.start_offset + extracted.end_offset,
                    excerpt=excerpt,
                )
                self._repository.save_assertion(item)

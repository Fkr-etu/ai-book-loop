...
class ChapterWorkflow:
    ...
    def _next_attempt(self, state: ChapterWorkflowState) -> int:
        """Allocate a version after the latest persisted version to make reruns idempotent."""
        persisted_next = self.repository.next_chapter_version(state.book.id, state.chapter_number)
        return max(state.attempt + 1, persisted_next)

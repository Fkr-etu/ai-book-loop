...
class ChapterWorkflow:
    ...
    def _review_from_linguistic_diagnostics(diagnostics: list[Diagnostic], *, unavailable_error: str | None = None) -> SceneReview:
        ...
        return SceneReview(score=0, approved=False, issues=issues, suggestions=suggestions or ["Correct the blocking linguistic diagnostics before review."])
    ...
    def _linguistic_review(self, state: ChapterWorkflowState) -> tuple[SceneReview | None, list[Diagnostic]]:
        if self.linguistic_validator_factory is None:
            return None, []
        result = self.linguistic_validator_factory(state.book).validate(state.draft, language=self.linguistic_language)
        ...
        if result.status == LinguisticCheckStatus.CHECK_NOT_AVAILABLE:
            return self._review_from_linguistic_diagnostics(diagnostics, unavailable_error=result.error or "unknown checker failure"), diagnostics
    ...
    def _review_impl(self, state: ChapterWorkflowState) -> dict:
        ...
        if not lint.valid:
            review = SceneReview(score=0, approved=False, issues=lint.errors, suggestions=["Remove all lint errors before the next review."])
        else:
    ...
    def _run_stepwise(self, *, book: BookState, run: ChapterWorkflowRun) -> ChapterWorkflowState:
        ...
        while run.status == WorkflowRunStatus.RUNNING:
            ...
            if run.step == WorkflowStep.REVIEW:
                state = self._state_from_run(book, run)
                result = self._review(state) if run.review is None else {"decision": run.decision, "review_score": run.review.score, "review": run.review}
                run.review = result["review"]
                run.decision = result["decision"]
                run.step = WorkflowStep.SUMMARIZE if run.decision == ReviewDecision.ACCEPT.value else WorkflowStep.CORRECT if run.decision == ReviewDecision.RETRY.value else WorkflowStep.REVIEW
                if run.step == WorkflowStep.REVIEW:
            ...
            if run.step == WorkflowStep.SUMMARIZE:
                if run.summary is None:
                    run.summary = self.summarizer.summarize(context=self.context_builder.for_chapter(book, run.chapter_number), chapter=run.draft)

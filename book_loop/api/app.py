...
from book_loop.domain.models import BookState, Chapter, ChapterStatus, User, UserPublic, Outline
...
def create_app(container: Container | None = None) -> FastAPI:
    ...
    def register(payload: RegisterPayload, response: Response) -> dict[str, Any]:
        ...
        new_user = User(id=user_id, email=payload.email, password_hash=hashed, name=payload.name)
        ...
        response.set_cookie(key=COOKIE_NAME, value=token, httponly=True, secure=False, samesite="lax", path="/", max_age=7 * 24 * 3600)
    ...
    def _get_or_seed_book(book_id: str) -> BookState:
        try:
            ...
            default_book = BookState(
                id="proj-001", title="L'Écho du Codex", theme="Mystères alchimiques et cités perdues",
                ...
                constraints=["Interdire les anachronismes modernes", "Conserver une voix narrative érudite à la 3ème personne", "Maintenir des descriptions tactiles"],
                ...
                chapters=[
                    Chapter(id="chap-1", number=1, title="Le Murmure du Parchemin", objective="Découvrir la tablette d'obsidienne dans les archives scellées.", status=ChapterStatus.APPROVED, current_version=1, summary="Chapitre 1: Découverte de la tablette d'obsidienne [Canonique]"),
                    Chapter(id="chap-2", number=2, title="La Cité Suspendue", objective="Ascension des tours mnésiques jusqu'au pont de verre.", status=ChapterStatus.NEEDS_REVIEW, current_version=1, summary=""),
                ],
            )
    ...
    def get_book(book_id: str) -> dict[str, Any]:
        return _get_or_seed_book(book_id).model_dump(mode="json")

    @app.post("/api/books")
    def create_book(payload: CreateBookPayload) -> dict[str, Any]:
        book = container.create_book().execute(title=payload.title, theme=payload.theme, author_idea=payload.author_idea, lore=payload.lore, constraints=payload.constraints)
    ...
    def generate_outline(book_id: str) -> dict[str, Any]:
        return container.generate_outline().execute(_get_or_seed_book(book_id)).model_dump(mode="json")

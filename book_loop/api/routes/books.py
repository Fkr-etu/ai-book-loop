...
def _serialize_book(book: Any, container: Container) -> dict[str, Any]:
    ...
    for chapter in response["chapters"]:
        ...
        chapter["versions"] = [{"id": version["id"], "versionNumber": version["version"], "content": version["draft"], "createdAt": version["created_at"], "source": "ai", "status": chapter["status"]} for version in versions]
    return response
...
def list_books(request: Request, container: Container = Depends(get_container)) -> list[dict[str, Any]]:
    ...
    return [_serialize_book(book, container) for book in container.repository.list_books_for_owner(current_user.id)]
...
def delete_book(book_id: str, payload: DeleteBookPayload, request: Request, container: Container = Depends(get_container)) -> None:
    book = get_owned_book(book_id, request, container)
    if payload.title != book.title:
        raise HTTPException(status_code=400, detail="Pour confirmer la suppression, saisissez exactement le titre du livre.")
    try:
        container.repository.delete_book(book.id)

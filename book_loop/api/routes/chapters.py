...
def review_chapter(book_id: str, chapter_number: int, payload: ReviewPayload = Body(default_factory=ReviewPayload), container: Container = Depends(get_container)) -> dict[str, Any]:
    ...
    except ValueError as exc: raise HTTPException(status_code=404 if "not found" in str(exc).lower() else 400, detail=str(exc))
...
def approve_chapter(book_id: str, chapter_number: int, container: Container = Depends(get_container)) -> dict[str, Any]:
    book = get_book(book_id, container)
    try: result = container.approve_chapter_and_sync_canon().execute(book, chapter_number=chapter_number)
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc))
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc))
    response = result.book.model_dump(mode="json")
    response["canonSync"] = {"sourceDocument": result.ingestion.source_document.model_dump(mode="json"), "assertionCount": len(result.ingestion.assertions), "evidenceCount": len(result.ingestion.evidence), "conflicts": [conflict.model_dump(mode="json") for conflict in result.conflicts]}
    return response

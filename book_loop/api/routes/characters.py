...
def _owned_book(book_id: str, container: Container = Depends(get_container)) -> BookState:
    # Ownership is enforced by the shared API dependency; the returned book is unused here.
    raise RuntimeError("_owned_book must be overridden by FastAPI dependency resolution")

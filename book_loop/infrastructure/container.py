...
class Container:
    ...
    def _linguistic_validator(self, book):
        ...
        if mode in {"", "disabled", "off", "none"}: return LinguisticValidationService(())
        checkers = []
        if mode in {"languagetool", "both", "all"}: checkers.append(LanguageToolChecker(base_url=self.settings.language_tool_url))
        if mode in {"spacy", "both", "all"}: checkers.append(SpacyFrenchChecker(model_name=self.settings.spacy_model))
        ...
        if not checkers: raise ValueError("Unsupported LINGUISTIC_CHECKER value; use disabled, languagetool, spacy, canon, both or all")

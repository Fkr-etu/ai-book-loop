...
class SQLiteBookRepository:
    def __init__(self, database_url: str) -> None:
        ...
        self._connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS chapter_versions (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id TEXT NOT NULL, chapter_number INTEGER NOT NULL, version INTEGER NOT NULL, draft TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(book_id, chapter_number, version));
            CREATE TABLE IF NOT EXISTS reviews (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id TEXT NOT NULL, chapter_number INTEGER NOT NULL, version INTEGER NOT NULL, score INTEGER NOT NULL, approved INTEGER NOT NULL, issues TEXT NOT NULL, suggestions TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
            ...
            CREATE TABLE IF NOT EXISTS assertions (id TEXT PRIMARY KEY, source_document_id TEXT NOT NULL, chunk_id TEXT NOT NULL, statement TEXT NOT NULL, subject TEXT NOT NULL, predicate TEXT NOT NULL, object TEXT NOT NULL, confidence REAL NOT NULL, status TEXT NOT NULL, evidence_id TEXT NOT NULL);
            ...
            CREATE TABLE IF NOT EXISTS canonical_facts (id TEXT PRIMARY KEY, book_id TEXT NOT NULL, assertion_id TEXT NOT NULL, statement TEXT NOT NULL, subject TEXT NOT NULL, predicate TEXT NOT NULL, object TEXT NOT NULL, decision_id TEXT NOT NULL, version INTEGER NOT NULL, active INTEGER NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(book_id, subject, predicate, version));
            """
        )
    ...
    def save_chapter_version(self, book_id: str, chapter_number: int, version: int, draft: str) -> None:
        self._connection.execute("INSERT INTO chapter_versions(book_id, chapter_number, version, draft) VALUES(?, ?, ?, ?)", (book_id, chapter_number, version, draft))
        self._connection.commit()

    def save_review(self, book_id: str, chapter_number: int, version: int, review: SceneReview) -> None:
        self._connection.execute("INSERT INTO reviews(book_id, chapter_number, version, score, approved, issues, suggestions) VALUES(?, ?, ?, ?, ?, ?, ?)", (book_id, chapter_number, version, review.score, int(review.approved), json.dumps(review.issues), json.dumps(review.suggestions)))
        self._connection.commit()

    def find_source_by_hash(self, *, book_id: str, content_hash: str) -> SourceDocument | None:
        row = self._connection.execute("SELECT * FROM source_documents WHERE book_id = ? AND content_hash = ?", (book_id, content_hash)).fetchone()
        return self._source_from_row(row) if row is not None else None

    def save_source(self, source: SourceDocument) -> None:
        self._connection.execute("INSERT INTO source_documents(id, book_id, name, source_type, content, content_hash, metadata, version) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", (source.id, source.book_id, source.name, source.source_type, source.content, source.content_hash, json.dumps(source.metadata), source.version))
        self._connection.commit()

    def save_chunk(self, chunk: DocumentChunk) -> None:
        self._connection.execute("INSERT INTO document_chunks(id, source_document_id, content, sequence, start_offset, end_offset, metadata) VALUES(?, ?, ?, ?, ?, ?, ?)", (chunk.id, chunk.source_document_id, chunk.content, chunk.sequence, chunk.start_offset, chunk.end_offset, json.dumps(chunk.metadata)))
        self._connection.commit()

    def save_assertion(self, assertion: Assertion) -> None:
        self._connection.execute("INSERT INTO assertions(id, source_document_id, chunk_id, statement, subject, predicate, object, confidence, status, evidence_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (assertion.id, assertion.source_document_id, assertion.chunk_id, assertion.statement, assertion.subject, assertion.predicate, assertion.object, assertion.confidence, assertion.status.value, assertion.evidence_id))
        self._connection.commit()

    def save_evidence(self, evidence: Evidence) -> None:
        self._connection.execute("INSERT INTO evidence(id, assertion_id, source_document_id, chunk_id, start_offset, end_offset, excerpt) VALUES(?, ?, ?, ?, ?, ?, ?)", (evidence.id, evidence.assertion_id, evidence.source_document_id, evidence.chunk_id, evidence.start_offset, evidence.end_offset, evidence.excerpt))
        self._connection.commit()

    def list_assertions(self, *, book_id: str) -> list[Assertion]:
        rows = self._connection.execute("SELECT a.* FROM assertions a JOIN source_documents s ON s.id = a.source_document_id WHERE s.book_id = ? ORDER BY a.rowid", (book_id,)).fetchall()
    ...
    def save_conflict(self, conflict: Conflict) -> None:
        ...
        self._connection.execute("INSERT INTO conflicts(id, book_id, left_assertion_id, right_assertion_id, status, resolution_assertion_id) VALUES(?, ?, ?, ?, ?, ?) ON CONFLICT(left_assertion_id, right_assertion_id) DO UPDATE SET status=excluded.status, resolution_assertion_id=excluded.resolution_assertion_id", (conflict.id, conflict.book_id, left, right, conflict.status.value, conflict.resolution_assertion_id))
        self._connection.commit()
    ...
    def deactivate_canonical_facts(self, *, book_id: str, subject: str, predicate: str) -> None:
        self._connection.execute("UPDATE canonical_facts SET active = 0 WHERE book_id = ? AND subject = ? AND predicate = ? AND active = 1", (book_id, subject, predicate))
        self._connection.commit()

    def save_canonical_fact(self, fact: CanonicalFact) -> None:
        self._connection.execute("INSERT INTO canonical_facts(id, book_id, assertion_id, statement, subject, predicate, object, decision_id, version, active) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (fact.id, fact.book_id, fact.assertion_id, fact.statement, fact.subject, fact.predicate, fact.object, fact.decision_id, fact.version, int(fact.active)))
        self._connection.commit()

    def list_active_canonical_facts(self, *, book_id: str) -> list[CanonicalFact]:
        rows = self._connection.execute("SELECT * FROM canonical_facts WHERE book_id = ? AND active = 1 ORDER BY subject, predicate, version", (book_id,)).fetchall()
        return [CanonicalFact(id=row["id"], book_id=row["book_id"], assertion_id=row["assertion_id"], statement=row["statement"], subject=row["subject"], predicate=row["predicate"], object=row["object"], decision_id=row["decision_id"], version=row["version"], active=bool(row["active"])) for row in rows]
